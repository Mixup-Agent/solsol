...
> 📌 **이 문서는 해커톤 당시의 시간대별 계획 스냅샷(기록용)입니다.**
> 실제 구현은 이 계획과 다르게 진행됐습니다 — FastAPI를 포기하지 않고 유지했으며,
> 단일 스크립트가 아닌 **LangGraph 멀티에이전트 + FastAPI** 구조로 구현됐습니다.
> 아래 코드 예시는 구버전(클래스 기반 `agents/`)이라 현재 코드와 맞지 않습니다.
> 현행 구조·실행 방법은 [interview-agent.md](./interview-agent.md)를 참고하세요.
